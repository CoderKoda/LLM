...
class Attention(nn.Module):
    def __init__(self, c: GPTConfig):
        super().__init__()
        if c.n_embd % c.n_head: raise ValueError("n_embd must be divisible by n_head")
        self.heads = c.n_head
        self.qkv = nn.Linear(c.n_embd, 3*c.n_embd)
        self.proj = nn.Linear(c.n_embd, c.n_embd)
        self.dropout = c.dropout
    def forward(self, x):
        b,t,c = x.shape; d = c // self.heads
...
class Block(nn.Module):
    def __init__(self,c):
        super().__init__(); self.ln1=nn.LayerNorm(c.n_embd); self.attn=Attention(c); self.ln2=nn.LayerNorm(c.n_embd)
        self.mlp=nn.Sequential(nn.Linear(c.n_embd,4*c.n_embd),nn.GELU(),nn.Linear(4*c.n_embd,c.n_embd),nn.Dropout(c.dropout))
    def forward(self,x): return x+self.attn(self.ln1(x)) if False else x+self.mlp(self.ln2(x+self.attn(self.ln1(x))))

class GPT(nn.Module):
    def __init__(self,c):
        super().__init__(); self.config=c
        self.tok=nn.Embedding(c.vocab_size,c.n_embd); self.pos=nn.Embedding(c.block_size,c.n_embd)
        self.drop=nn.Dropout(c.dropout); self.blocks=nn.ModuleList(Block(c) for _ in range(c.n_layer)); self.ln=nn.LayerNorm(c.n_embd)
        self.head=nn.Linear(c.n_embd,c.vocab_size,bias=False); self.head.weight=self.tok.weight; self.apply(self._init)
    @staticmethod
    def _init(m):
    ...
    def generate(self,idx,max_new_tokens,temperature=0.8,top_k=40):
        ...
        for _ in range(max_new_tokens):
            ...
            if top_k:
                k=min(top_k,logits.size(-1)); values,_=torch.topk(logits,k); logits[logits<values[:,-1,None]]=-float('inf')
